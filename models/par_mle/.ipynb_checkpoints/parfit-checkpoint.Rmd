---
title: "Parameter fitting using POMP"
...
Table of Contents:
1. Entering and cleaning data
2. Compartmental model
3. Global MLE and running simulations
4. Comparing the simulated outbreak with expected data
...
# Expected cases
case_data <- read.csv("test/cases.csv")
...
stew(file="box_search_global_with_intervention.rda",{
  w2 <- getDoParWorkers()
  t2 <- system.time({
...
write.csv(global_params,file="global_params.csv",row.names=FALSE,na="")
```
...
print(theta_global)
write.csv(theta_global,file="best_params.csv",row.names=TRUE,na="")

m1  %>%
  simulate(params=c(theta_global, fixed_params),nsim=9,
             format = "data.frame",include.data=TRUE) -> sim_data

s1 = subset(sim_data, .id == 1)

#m1  %>%
#  simulate(params=c(theta_global, fixed_params), nsim=9,as.data.frame=TRUE,include.data=TRUE) %>%
sim_data %>%
